...
class feature_ents():
    ...
    def combine_features(self, news):
        content_words, title_words = self.get_tfidf_Score(news)
        keys = content_words.keys()|content_words.keys()
        features = []
        for ner in keys:
            features.append([[ner],[content_words[ner] if ner in content_words else 0,
                                    title_words[ner] if ner in title_words else 0,
                                    len(ner)]]) # 特征：正文中的tfidf，标题中的tfidf，实体的长度
            # feature_dict[ner]['f1'] = content_words[ner]
